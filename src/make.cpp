...
void write_output_groups(std::ostream &output, const DependencyFacts &facts)
{
    std::vector<std::pair<std::filesystem::path, std::filesystem::path>> groups;
    for (const TranslationUnit &unit : facts.translation_units)
    {
        if (unit.provides.size() == 1 && !unit.object_path.empty())
        {
    ...
    for (const auto &[bmi, object] : groups)
    {
...
void write_module_mappings(std::ostream &output, const std::filesystem::path &target,
                           std::string_view variable, const std::vector<ModuleImport> &modules)
{
    output << escaped_path(target) << ": " << variable << " :=";
    for (const ModuleImport &module : modules)
    {
...
void write_make(std::ostream &output, const DependencyFacts &facts)
{
    ...
    write_output_groups(output, facts);
    ...
    for (const TranslationUnit *unit : units)
    {
        ...
        for (const ProvidedModule &module : provided)
        {
            ...
            write_target_variable(output, module.bmi_path, "CXX_MODGRAPH_IMPORT_BMIS",
                                  imported_bmis);
            write_module_mappings(output, module.bmi_path, "CXX_MODGRAPH_PROVIDES",
                                  {{module.name, module.bmi_path}});
            write_module_mappings(output, module.bmi_path, "CXX_MODGRAPH_IMPORTS", imports);
